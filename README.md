...
# About this Project
This is a simple frontend application that allows user to explore art from the Art Institue of Chicagos repositiory. It allows the user to chose from 6 per
# Built With
...
# Optimizations 
-Refactoring this code in React would be ideal, it would allows for less redundant code and over all better user. 

-Additional error handling could be added to manage cases where the images are not found, which is difficult to add due to the way the image are being dynamical displayed in JS.

-Adding more information about the artist, the type of art, and related art pieces.
 
-There is a randomization feature added to the api request, this was added to migitate the "boosted" results that returned similar result for different query request. In the future, implementing a more targeted approach that would check the results for redundancy before displaying them. 

-Fullstack Optimizations: allow user to save artwork and go back to to saved artwork pieces.

# Limitations
-This website is only functional with browsers that can run JS. In the future I would like to add static images to allow individual who do not have access to modern web browsers the opportunity to experience different are work.
